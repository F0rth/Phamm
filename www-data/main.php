...
// Set default language from user browser
...
if (isset($_GET["exit"]))
{
    phamm_log ($_SESSION["phamm"]["pn"],$_SESSION["login"]["username"],'logout',1);
...
elseif(login_check())
{
    ...
    phamm_print_xhtml ("<a href=\"?exit=1\">");
    printf (_("Logout %s"),$_SESSION["login"]["username"]);
    phamm_print_xhtml ('</a></td><td align="right">');
	...
	phamm_print_message('success',sprintf(_("%s modified"),$k));
...
case "insert_domain" :
	...
	phamm_log ($pn,$_SESSION["login"]["username"],$action.' '.$domain_new.' '.$wrong_pw_msg,0);
    }
	...
	$values_purged = purge_empty_values($entry); // @todo 2th value
        ...
        $r_d = @phamm_add ('vd='.$domain_new.','.LDAP_BASE,$values_purged);
    
	phamm_log ($pn,$_SESSION["login"]["username"],$action.' '.$domain_new,$r_d);
	...
	// Add postmaster alias
        $r_p = @phamm_add ('cn=postmaster,vd='.$domain_new.','.LDAP_BASE,$entry);
	...
	// Add abuse alias
        $r_a = @phamm_add ('mail=abuse@'.$domain_new.',vd='.$domain_new.','.LDAP_BASE,$entry_abuse);
...
case "add_account" :
    ...
    $account_exists = phamm_list('vd='.$domain.','.LDAP_BASE,"(mail=".$_POST["account_new"]."@".$domain.")",array('mail'));

    if ($syntax && $account_exists["count"] == 0)
...
case "domain_view" :
    ...
    phamm_print_xhtml ( '<a href="?action=domain_property&amp;domain='.$domain.'">'._("Domain property").'</a> ' );
	...
	if (isset($values[0]["mail"][0]))
	    phamm_print_message('warning','<br/>'._("Catch all active"));
	else
	    phamm_print_xhtml ( ' - <a href="?action=catch_all&amp;domain='.$domain.'">'._("Catch all").'</a>' );
    }
    ...
    phamm_print_xhtml ( accounts_list($domain,$values,$initial) );
...
case "insert_account" :
	...
	phamm_log ($pn,$_SESSION["login"]["username"],$action.' '.$account_new.' '.$wrong_pw_msg,0);
    }
...
case "update_account" :
	...
	phamm_log ($pn,$_SESSION["login"]["username"],$action.' '.$wrong_pw_msg,0);
	...
	$r_d = phamm_modify ('vd='.$domain.','.LDAP_BASE,$entry_domain);
	
	phamm_log ($pn,$_SESSION["login"]["username"],'update_domain '.$domain,$r);
    }
...
case "domain_property" :
	...
	$values = phamm_search ($dn,$filter="objectClass=VirtualDomain");
	
	$dn_postmaster = 'cn=postmaster,vd='.$domain.','.LDAP_BASE;
	$values_postmaster = phamm_search ($dn_postmaster,$filter="cn=postmaster");
